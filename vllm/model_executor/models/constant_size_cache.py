...
class ConstantSizeCache(ABC):
    ...
    def _assign_seq_id_to_cache_index(self, cur_rid: str, seq_id: int,
        ...
        if cur_rid in finished_requests_ids:
            ...
            destination_index = self._wait_and_pop_free_index()
    ...
    def _wait_and_pop_free_index(self, timeout: float = 60.0, interval: float = 1.0) -> int:
        """
        Waits up to `timeout` seconds until a free cache index is available,
        then pops and returns it. Raises RuntimeError if timeout is exceeded.
        """
        start = time.time()
        while not self.free_cache_indices:
            if time.time() - start > timeout:
                raise RuntimeError(
                    "[ConstantSizeCache] Timed out waiting for a free cache index. "
                    "This may indicate a race condition or undersized cache."
                )
            time.sleep(interval)
        return self.free_cache_indices.pop()
